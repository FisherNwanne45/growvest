<template>
  <main class="container flex-grow p-4 sm:p-6">
    <PageHeader title="Partners" :segments="segments" :buttons="buttons" />
    <div class="space-y-6">
              ...
              <table class="table">
                ...
                <tbody>
                  <tr v-for="row in brands.data" :key="row.id">
                    <td class="text-left">
                      <img v-lazy="row.slug" class="avatar rounded-square w-70-per" />
                    </td>
                    <td class="text-left">
                      {{ row.title }}
                    </td>
    ...
    <form @submit.prevent="updatePartner()">
      ...
      <div class="drawer-body">
        ...
        <div class="mb-2">
          <label>{{ trans('Brand image') }}</label>
          <input
            @input="(e) => (editPartnerForm.image = e.target.files[0])"
            type="file"
            name="image"
            accept="image/*"
            class="input"
