...
const props = defineProps(['segments', 'buttons', 'categories', 'project', 'metas'])
...
const form = useForm({
  ...
  min_return: props.project?.min_return,
  invest_amount: props.project?.invest_amount,
  expire_date: props.project?.expire_date,
  address: props.project?.address,
  durations: props.project?.durations ?? [{ ...newDurationItem }],

  main_description: props.metas?.main_description,
  short_description: props.metas?.short_description,

  seo_title: props.metas?.seo_title,
  seo_image: '',
  seo_description: props.metas?.seo_description,
  seo_tags: props.metas?.seo_tags,

  faqs: props.project.faqs ?? [{ ...newFaqItem }],

  accept_new_investor: props.project?.accept_new_investor,
...
<template>
  <main class="container flex-grow p-4 sm:p-6">
    <PageHeader title="Edit Project" :segments="segments" :buttons="buttons" />
