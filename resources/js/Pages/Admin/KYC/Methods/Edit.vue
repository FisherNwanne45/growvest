...
const { kycMethod, types, segments, buttons } = defineProps(['types', 'kycMethod', 'segments', 'buttons'])

const form = useForm({ ...kycMethod })
form.image = null

const submit = () => {
    form.put(route('admin.kyc-methods.update', kycMethod), {
        onSuccess: () => {
...
<template>
    <main class="container p-4 sm:p-6">
        <PageHeader :title="trans('KYC Methods Create')" :segments="segments" :buttons="buttons" />
                ...
                <div class="col-1">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <SpinnerBtn classes="btn btn-primary" :processing="form.processing" btn-text="Save Changes" />
                        </div>
                    </div>
                </div>
