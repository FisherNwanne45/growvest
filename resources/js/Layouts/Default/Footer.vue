...
<template>
  <footer class="bg-black bg-[url('../images/all-img/footer-bg-1.png')] bg-cover bg-center bg-no-repeat">
      ...
      <div class="grid grid-cols-1 gap-7 md:grid-cols-2 lg:grid-cols-3">
        <div class="single-footer">
          <div class="lg:max-w-[270px]">
            ...
            <p>{{ $page.props?.primaryData?.footer_description }}</p>
            <ul class="flex space-x-4 pt-8">
              <li v-for="(social, key) in socials" :key="key">
                <a v-if="social" :href="social" target="_blank"
        ...
        <div class="single-footer col-span lg:col-span-1"
          ...
          <template v-if="NEWSLETTER_API">
            ...
            <form @submit.prevent="subscribe">
              <div class="shadow-e1 mb-4 flex items-center rounded-md bg-white py-[10px] pl-6 pr-[10px]">
                <div class="flex-none">
                  <span class="">
                ...
                <div class="flex-1">
                  <input type="email" v-model="subscribeForm.email" :placeholder="trans('Enter your mail')"
                    class="border-none focus:ring-0" required />
                </div>
              </div>
    ...
    <div class="container border-t border-white border-opacity-[0.1] py-8 text-center text-base"
      v-html="$page.props.primaryData?.copyright_text"></div>
  </footer>
</template>
